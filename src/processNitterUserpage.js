...
exports.parseTwitterUserPage = async function(axiosResponse, options) {
	...
	try {
		...
		try {
			...
			logModule.log(logModule.LOG_LEVEL_ERROR, 'Error Downloading file: ' + profileInfo.avatarThumb);
		...
		if(protectedTitle != null) {
			...
			return {'profile' : profileInfo, 'timeline': null, 'entireTimeline': null};
		}
		...
		let allsrcElements = entireTimeline.querySelectorAll('[src]');
		...
		let alldataurlElements = entireTimeline.querySelectorAll('[data-url]');
